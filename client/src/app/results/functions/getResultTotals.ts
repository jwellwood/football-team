...
export const getResultTotals = (results: IResult[]) => {
  ...
  const wins: number = results.filter((res) => +res.points === 3).length;
  const draws: number = results.filter((res) => +res.points === 1).length;
  const losses: number = results.filter((res) => +res.points === 0).length;
